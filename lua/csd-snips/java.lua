...
M.get_debug = function(string_input)
	...
	local append_param_list = closure_param_list(vars_table, append_comma_and_var)
	local generate_message_format = utils.decorate_three_param_function(utils.reducer, vars_table, get_var_string)
...
M.get_debug_with_notation = function(string_input)
	...
	local get_var_string = closure_message_format()
	local append_param_list = closure_param_list(vars_table, append_comma_and_var_notation)
	local generate_message_format = utils.decorate_three_param_function(utils.reducer, vars_table, get_var_string)
